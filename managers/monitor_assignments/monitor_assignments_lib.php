...
function monitor_assignments_get_professionals_by_instance( $instance_id ){
    ...
    AND estado = 1
    AND id_semestre =". core_periods_get_current_period($instance_id)->id ." ORDER BY fullname";
...
function monitor_assignments_get_profesional_practicant_relationship_by_instance( $instance_id ){
	...
	WHERE user_rol_1.id_semestre = ". core_periods_get_current_period($instance_id)->id;
...
function monitor_assignments_create_monitor_student_relationship( $instance_id, $monitor_id, $student_id ){
    ...
    $current_id_semester = core_periods_get_current_period($instance_id)->id;
...
function monitor_assignments_transfer( $instance_id, $old_monitor_id, $new_monitor_id ){
    ...
    $sql = "SELECT * 
            FROM {talentospilos_monitor_estud} 
            WHERE id_semestre = $current_id_semester AND id_instancia = $instance_id AND id_monitor = $old_monitor_id";
    ...
    if( $asignations ){
        ...
        foreach($asignations as &$asignation){
            ...
            if($asignation->id == 0){
                trigger_error('ASES Notificacion: actualizar asignacion en la BD con id 0');
            }else{
