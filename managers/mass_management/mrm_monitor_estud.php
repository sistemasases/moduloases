...
require_once __DIR__ . '/massmanagement_lib.php';
require_once __DIR__ .'/../user_management/user_lib.php';

if (isset($_POST) && isset($_FILES['file'])) {
    return upload_assignations_file($_FILES['file'], $_POST['idinstancia']);
}

/**
 * Reads $file and performs the specified monitor->student
 * assignations.
 *
 * @author David S. Cortés - 2021 <david.cortes@correounivalle.edu.co>
 *
 * @param $instance_id instance where the assignations take place.
 *
 */
function upload_assignations_file($file, $instance_id) {
    ...
    if (is_numeric($instance_id)) {
        try {
            ...
            while ($data = fgetcsv($handle, 0, ",")) {
                ...
                $last_semester = core_periods_get_last_period($instance_id);
    
                //validating duplicity on instance
                $sql_query = "SELECT *  FROM {talentospilos_monitor_estud} where id_semestre='" . $last_semester->id . "' AND id_estudiante='" . $id_estudiante . "' AND id_instancia='" . $instance_id . "'";
        ...
        return json_encode('File and/or instance id missing from request.');
    }
}
