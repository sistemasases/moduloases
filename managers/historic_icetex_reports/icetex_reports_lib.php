...
function get_resolutions_for_report(){
    ...
    $sql_query = "SELECT DISTINCT res_ice.id, res_ice.codigo_resolucion, semestre.nombre, 
                    ...
                    FROM {talentospilos_res_icetex} AS res_ice
...
function get_count_active_res_students($cohort){
    ...
    $original_sql_query = "SELECT semestre.nombre AS semestre, Count(res_est.id) AS num_act_res, sum(res_est.monto_estudiante) AS monto_act_res
