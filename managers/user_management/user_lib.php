...
function changeMonitor($oldMonitor, $newMonitor)
{
    ...
    try {
        $lastsemester = core_periods_get_current_period();
        ...
        foreach ($result as $row) {
            ...
            if($newObject->id == 0){
                trigger_error('ASES Error: actualizar monitor en la BD con id 0');
            }
...
function update_role_monitor_ps($username, $role, $array_students, $boss, $idinstancia, $state = 1)
{
    ...
    if (empty($id_rol_actual)) {
        ...
        if($object_role->id == 0){
            trigger_error('ASES Error: actualizar rol en la BD con id 0');
        }
...
function manage_role_profesional_ps($username, $role, $professional, $idinstancia, $state = 1){
    ...
    try {
        ...
        $id_current_semester = core_periods_get_current_period();
        ...
        if (empty($id_current_role)) {
            ...
            if ($object_user_role) {
                ...
                if ($id_current_role->nombre_rol == 'monitor_ps') {
                    ...
                    $lastsemester = core_periods_get_last_period();
...
function update_professional_user($id_user, $professional)
{
    ...
    if ($id_professional) {
        ...
        if($record_professional_type->id == 0){
            trigger_error('ASES Error: actualizar profesional en la BD con id 0');
        }
