...
select
       count(*) filter(where item_ganado = false) as cantidad_items_perdidos,
       count(*) filter (where item_ganado = true) as cantidad_items_ganados,
       username, mdl_talentospilos_usuario_id,
       firstname,
       lastname ,
num_doc  from (
select
...
/* Consulta de todos los estudiantes, practicantes jefes y monitores*/
-- La inforamción debe corresponder con lo mostrado en adignación monitor
-- ejemplo de url de asignación monitor: wwwroot./moodle35/blocks/ases/view/monitor_assignments.php?courseid=25643&instanceid=450299
...
where
 ...
 and mdl_talentospilos_instancia.id_instancia = 450299
and mdl_talentospilos_semestre.nombre = '2018B';
