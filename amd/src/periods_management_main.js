	...
	return {
		...
		init: function () {
			...
			function validateSemesterFields(semesterName, beginningDateString, endingDateString, semesterId) {
				...
				var regexpFormat = /^\d{2}\s([aA-zZ]){3,12}\s\d{4}$/;

				var begin_regexpFormat_date = regexpFormat.exec(beginningDateString)
				var end_regexpFormat_date = regexpFormat.exec(endingDateString)

				if (begin_regexpFormat_date != null && end_regexpFormat_date != null) {
					beginningDateString = convertDate(beginningDateString)
					endingDateString = convertDate(endingDateString)
				}
				else if (begin_regexpFormat_date == null && end_regexpFormat_date != null) {
					endingDateString = convertDate(endingDateString)
				}
				else if (begin_regexpFormat_date != null && end_regexpFormat_date == null) {
					beginningDateString = convertDate(beginningDateString)
			 ...
			 * @method convertDate
			 * @desc Converts format dd-mm-yyyy to a yyyy-mm-dd
			 * @param {string} dateFormat date in format dd-mm-yyyy
			 * @return {string} date in format yyyy-mm-dd
			 * @example convertDate("01-may-2017")
			 * @example convertDate("01-may-2017")
			 */
			function convertDate(dateFormat) {
				//check the format
				var regexp = /^(\d{2})\s((enero|january)|(febrero|february)|(marzo|march)|(abril|april)|(mayo|may)|(junio|june)|(julio|july)|(agosto|august)|(septiembre|september)|(octubre|october)|(noviembre|november)|(diciembre|december))\s(\d{4})$/;
				//check that the input complies with the format
				var date = ""
				var begin_regexp_date = regexp.exec(dateFormat.toLowerCase());
				//if it is different from null it is because it complied with the format and the conversion will be done
				if (begin_regexp_date != null) {
					//the exec method returns a vector and from position 3 to 14 are the groups of the months that match their number in the calendar
					for (let i = 3; i < 15; i++) {

						if (begin_regexp_date[i] != null && i < 12) {
							date = '0' + (i - 2)
							break;
						} else if (i >= 12 && begin_regexp_date[i] != null) {
							date = i - 2
							break;
						}
					...
					return begin_regexp_date[15] + '-' + date + '-' + begin_regexp_date[1]
				}
